...
-- Corrected version without SELECT trigger
...
-- 4. Create audit trigger for data modifications (not SELECT)
...
AS $function$
BEGIN
  -- Log sensitive data modifications when not accessing own data
...
-- 5. Add audit trigger for modifications only
DROP TRIGGER IF EXISTS audit_sensitive_modifications_trigger ON public.profile_sensitive_data;
CREATE TRIGGER audit_sensitive_modifications_trigger
  AFTER INSERT OR UPDATE OR DELETE
  ON public.profile_sensitive_data
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_sensitive_data_modifications();
...
-- 8. Create function to log HR access for auditing purposes
CREATE OR REPLACE FUNCTION public.log_hr_sensitive_access(target_user_id uuid, access_reason text DEFAULT 'HR data access')
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
