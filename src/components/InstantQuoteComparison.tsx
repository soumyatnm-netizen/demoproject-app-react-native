...
const InstantQuoteComparison = () => {
  ...
  const [extractedDocumentIds, setExtractedDocumentIds] = useState<
    { documentId: string; type: string; carrier: string }[]
  >([]);
  ...
  const handleSaveClick = (id: string) => {
    setReportSections((prev) =>
      prev.map((section) => (section.id === id ? { ...section, content: tempEditContent } : section)),
    );
    setEditingSectionId(null);
    setTempEditContent("");
  ...
  const fetchCompanyLogo = async () => {
    try {
      ...
      if (signedUrl?.signedUrl) {
        setCompanyLogoUrl(signedUrl.signedUrl);
      }
    } catch (error) {
  ...
  const processPolicyWordings = async () => {
    ...
    try {
      ...
      setProcessingStep(
        `Processing ${policyWordingDocs.length} policy wording document${policyWordingDocs.length !== 1 ? "s" : ""}...`,
      );
      ...
      for (let i = 0; i < policyWordingDocs.length; i++) {
        ...
        const { data: preflightResult, error: preflightError } = await supabase.functions.invoke("preflight-classify", {
        ...
        const { data: processResult, error: processError } = await supabase.functions.invoke("process-policy-wording", {
          body: { documentId: docData.id },
        });
    ...
    try {
      ...
      input.onchange = async (e: any) => {
        ...
        try {
          ...
          const { data: classifyData, error: classifyError } = await supabase.functions.invoke("preflight-classify", {
            body: { documentId: docData.id },
          });
          ...
          const { data: extractData, error: extractError } = await supabase.functions.invoke(functionName, {
            body: { documentId: docData.id },
          });
          ...
          addStatusLog(
            `✓ Retrieved ${previousQuotes?.length || 0} previous quotes and ${previousWordings?.length || 0} previous wordings`,
            "success",
          );
      ...
      .map((ranking, index) => {
        ...
        const coverageScore =
          calculateCoverageScore(
            ...
            maxLimits.professional_indemnity,
            COVERAGE_WEIGHTS.professional_indemnity,
          ) +
          calculateCoverageScore(
            coverage.public_liability,
            maxLimits.public_liability,
            COVERAGE_WEIGHTS.public_liability,
          ) +
          calculateCoverageScore(
            coverage.employers_liability,
            maxLimits.employers_liability,
            COVERAGE_WEIGHTS.employers_liability,
          ) +
          calculateCoverageScore(coverage.cyber_data, maxLimits.cyber_data, COVERAGE_WEIGHTS.cyber_data) +
          calculateCoverageScore(
            coverage.product_liability,
            maxLimits.product_liability,
            COVERAGE_WEIGHTS.product_liability,
          );
    ...
    try {
      ...
      const response = await fetch("https://ijhiavpjobzfxoirhnux.supabase.co/functions/v1/generate-pdf-report", {
        ...
        headers: {
          ...
          apikey:
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlqaGlhdnBqb2J6ZnhvaXJobnV4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU2OTg0ODgsImV4cCI6MjA3MTI3NDQ4OH0.eRjLwH8fCkqaMNnW0R248u213qcBRBLYrc9ZGiAm2Z4",
        },
...
<body class="bg-gray-100">
      ...
      return (
        <div className="my-8 p-6 bg-indigo-50 border border-indigo-200 rounded-xl shadow-inner flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-indigo-600 mt-2">
            (This diagram visually illustrates the capacity difference between separate indemnity towers and a single
            aggregate limit.)
          </p>
        </div>
      );
  ...
  const generateProductSections = (comparisonData: any): string => {
    ...
    return comparisonData.product_level_breakdown
      .map((product: any, idx: number) => {
        ...
        <div class="policy-section mb-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ${carriers
                  .map((carrier: any, carrierIdx: number) => {
      ...
      <Card className="bg-gradient-to-r from-primary/10 via-primary/5 to-background border-primary/20">
        <CardHeader className="text-center pb-2">
          ...
          <CardDescription className="text-lg">
            Upload quotes and compare coverage instantly. CoverCompassAI analyses schedules, limits, clauses, and terms
            to rank quotes from best to worst.
          </CardDescription>
        </CardHeader>
      </Card>
                ...
                {coverageSections.map((section) => (
                  ...
                  >
                    <input
                      ...
                      className="rounded"
                    />
                    <label className="text-sm font-medium cursor-pointer flex-1">{section.label}</label>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  {uploadedQuotes.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-3 bg-gradient-to-r from-green-50 to-green-25 border border-green-200 rounded-lg"
                    >
                ...
                <Label htmlFor="policy-wording-upload" className="cursor-pointer">
                  <span className="text-sm font-medium">
                    {policyWordingDocs.length === 0
                      ? "Click to upload policy wording documents"
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  {policyWordingDocs.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-blue-25 border border-blue-200 rounded-lg"
                    >
                      <div className="flex items-center space-x-3">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="bg-muted/50 border border-border rounded-lg p-3 mb-4">
              <p className="text-xs text-muted-foreground">
                Produced by CoverCompass AI for comparison purposes only. Not advice or a recommendation. Always verify
                against original insurer documentation; the original documents prevail. Brokers must always refer to the
                full policy wording and schedule provided by the insurer.
              </p>
            </div>
            ...
            <Button
              ...
              disabled={
                !selectedClient || (uploadedQuotes.length === 0 && policyWordingDocs.length === 0) || isProcessing
              }
              size="lg"
              className="w-full"
            >
            ...
            {(!selectedClient || (uploadedQuotes.length === 0 && policyWordingDocs.length === 0)) && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                ...
                <ul className="text-xs text-yellow-700 mt-1 space-y-1">
                  ...
                  {uploadedQuotes.length === 0 && policyWordingDocs.length === 0 && (
                    <li>• Please upload at least one quote or policy wording in Step 2</li>
                  )}
                </ul>
              </div>
            )}
            ...
            {isProcessing && (
              <div className="space-y-3">
                ...
                <Progress
                  value={
                    uploadedQuotes.length > 0
                      ? Math.round((100 / (uploadedQuotes.length + 1)) * (uploadedQuotes.length > 0 ? 1 : 0))
                      : 33
                  }
                  className="w-full"
                />
                <p className="text-xs text-muted-foreground">
                  Using CoverCompassAI to analyse schedules, limits, exclusions, enhancements, and core policy
                  wording...
                </p>
                  ...
                  <div className="mt-4 border border-border rounded-lg overflow-hidden">
                    ...
                    <div className="max-h-48 overflow-y-auto bg-background">
                      {statusLog.map((log, index) => (
                        ...
                        >
                          <span className="font-mono text-[10px] text-muted-foreground/60 min-w-[60px]">
                            {log.time}
                          </span>
                ...
                <Button onClick={cancelAnalysis} variant="destructive" size="sm" className="w-full">
                ...
                onClick={async () => {
                  try {
                    ...
                    const generateHTML = () => {
                      ...
                      const productSectionsHTML =
                        comparisonData.product_comparisons
                          ?.map((product, productIdx) => {
                            const carriersHTML =
                              product.carrier_results
                                ?.map((carrier, carrierIdx) => {
                                  ...
                                  const standoutPointsHTML =
                                    carrier.standout_points
                                      ?.map((point) => {
    ...
    <div id="app" class="max-w-6xl mx-auto py-8 px-4 bg-white rounded-xl comparison-card">
        <header class="mb-10 pb-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                ...
                ${companyLogoUrl ? `<img src="${companyLogoUrl}" alt="Company Logo" class="h-16 w-16 object-contain rounded-full" crossorigin="anonymous" />` : ''}
            </div>
        </header>
            ...
            <p class="text-xs text-gray-500">
                This report is for comparison purposes only. Brokers must always refer to the full policy wording and schedule provided by the insurer.
                    ...
                    const response = await fetch(
                      "https://ijhiavpjobzfxoirhnux.supabase.co/functions/v1/generate-pdf-report",
                      {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
              ...
              <CardContent>
                <div className="space-y-2">
                  <p className="text-sm text-amber-900 font-medium mb-3">
                    Failed to extract data from {comparisonData.failed_documents.length} document
                    {comparisonData.failed_documents.length !== 1 ? "s" : ""}:
                  </p>
                  <ul className="space-y-2">
                    {comparisonData.failed_documents.map((doc: any, idx: number) => (
                      <li
                        key={idx}
                        className="flex items-start space-x-2 text-sm bg-white/60 p-3 rounded border border-amber-200"
                      >
                  ...
                  <div className="mt-4 p-3 bg-white/80 rounded border border-amber-200">
                    <p className="text-xs text-amber-800">
                      <strong>Note:</strong> The comparison below is based on successfully extracted documents only. For
                      a complete analysis, please re-upload the failed documents or contact support if the issue
                      persists.
                    </p>
                  </div>
                </div>
            ...
            <Card className="border-2 border-primary/20 bg-muted/30">
              <CardContent className="p-4">
                <p className="text-xs text-muted-foreground text-center">
                  Produced by CoverCompass AI for comparison purposes only. Not advice or a recommendation. Always
                  verify against original insurer documentation; the original documents prevail. Brokers must always
                  refer to the full policy wording and schedule provided by the insurer.
                </p>
              </CardContent>
            </Card>
          )}
                    ...
                    return summaryItems
                      ...
                      .map((item: any, idx: number) => {
                        ...
                        const allPremiums = summaryItems
                          .map((i: any) => {
                            const amt =
                              typeof i.premium_amount === "string"
                    ...
                    <CardTitle className="flex items-center space-x-3">
                      {(() => {
                        ...
                        return insurerInfo.logo ? (
                          <img src={insurerInfo.logo} alt={insurerInfo.altText} className="h-10 w-10 object-contain" />
                        ) : (
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div className="col-span-2 bg-green-50 border border-green-200 rounded p-2">
                            ...
                            <p className="font-semibold text-green-900">
                              {insurer.quote_metrics.total_premium || "Unknown"}
                            </p>
                          </div>
                      ...
                      <div>
                        ...
                        <ul className="space-y-2">
                          {insurer.wording_highlights.map((highlight: any, idx: number) => (
                            <li
                              key={idx}
                              className="text-sm flex items-start bg-blue-50/50 rounded p-2 border border-blue-100"
                            >
          ...
          {comparisonData.quote_table && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Side-by-side comparison of all key quote metrics</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
          ...
          {comparisonData.overall_flags && comparisonData.overall_flags.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Important distinctions to consider</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
          ...
          {comparisonData.broker_report_markdown && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Full analysis for internal review</CardDescription>
              </CardHeader>
              <CardContent>
                <div
          ...
          {comparisonData.client_report_markdown && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Client-ready summary in plain English</CardDescription>
              </CardHeader>
              <CardContent>
                <div
                  className="prose prose-sm max-w-none"
        ...
        <>
          ...
          <Card data-section="coverage-highlights">
            <CardHeader>
              ...
              <CardDescription>Key coverage limits and which quote provides the best protection</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card data-section="quote-rankings">
            <CardHeader>
              ...
              <CardDescription>Ranked by overall coverage quality, competitiveness, and value</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold">£{ranking.premium_amount?.toLocaleString() || "N/A"}</div>
        ...
        <div className="mt-10 animate-in fade-in duration-500 max-w-7xl mx-auto w-full">
          ...
          <div ref={reportRef} className="space-y-6">
            ...
            <Card className="w-full shadow-sm border border-gray-200 rounded-xl overflow-hidden">
              <CardHeader className="pb-4 pt-5 bg-gradient-to-r from-primary/5 to-white border-b px-6">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-2xl font-bold text-primary">Policy Comparison Report</CardTitle>
                    <p className="text-sm text-muted-foreground mt-1">
                      Generated by Cover Compass | {new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
                    </p>
                    {selectedClient && (
                      <p className="text-sm font-medium text-foreground mt-1">
                        Client: {clients.find(c => c.id === selectedClient)?.client_name || "N/A"}
                      </p>
                    )}
                  </div>
                  {companyLogoUrl && (
                    <img
                      src={companyLogoUrl}
                      alt="Company Logo"
                      className="h-16 w-16 object-contain rounded-full border border-border shadow-sm"
                    />
                  )}
                </div>
              </CardHeader>
                    ...
                    <div className="prose prose-sm max-w-none dark:prose-invert text-slate-700 dark:text-slate-300 leading-relaxed">
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          tbody: ({ node, ...props }) => <tbody className="border-t border-gray-200" {...props} />,
                          th: ({ node, ...props }) => (
                            <th
                              className="px-4 py-2 border-b bg-gray-50 text-sm font-semibold text-gray-700"
                              {...props}
                            />
                          ),
                          td: ({ node, ...props }) => (
                            <td
                              className="px-4 py-4 align-top text-sm text-gray-700 whitespace-pre-line break-words"
                              {...props}
                            />
                          ),
                        }}
